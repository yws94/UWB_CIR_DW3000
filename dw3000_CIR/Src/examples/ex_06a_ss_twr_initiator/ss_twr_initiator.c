        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
            ...
            if (frame_len <= sizeof(rx_buffer))
            {
                ...
                if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
                {
                    ...
                    float clockOffsetRatio, rtd_resp_corr;
                    ...
                    rtd_resp_corr = rtd_resp * (1 - clockOffsetRatio);
                    //tof_prev=(rtd_init - rtd_resp)  * DWT_TIME_UNITS;
                    tof_prev=(rtd_init - rtd_resp);
                    ...
                    snprintf(test_buffer, sizeof(test_buffer), "poll_tx_ts: %u, resp_rx_ts: %u, poll_rx_ts: %u, resp_tx_ts: %u, rtd_init: %d, rtd_resp: %d, rtd_resp_corr: %f, clockOffsetRatio: %f,tof_prev: %lf, tof: %lf, Distance: %3.2f m\r\n",poll_tx_ts, resp_rx_ts, poll_rx_ts, resp_tx_ts, rtd_init, rtd_resp, rtd_resp_corr, clockOffsetRatio, tof_prev, tof, distance);

                    test_run_info((unsigned char *)test_buffer);
